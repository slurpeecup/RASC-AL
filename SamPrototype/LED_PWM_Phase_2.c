...
void timer2_init()
{
	// TCCR2A = 10xx--x11
	// TCCR2B = XX--XXXX
TCCR2A |= 163 << WGM20; //COM2A1, COM2B1, WGM20,WGM21	
TCCR2B |= 5<<CS20; // 111, PSC = 1024
OCR2A = 0;
}
...
void pin_init()
{
	DDRC  &= ~(1<<DDC0 | 1<< DDC1 | 1 << DDC2 
					   | 1<< DDC3 | 1 << DDC4);						 //Enable pin inputs C0, C1, C2, and C5 for buttons 
                                                                     //	PC0 (black button) controls increment up, PC1 (blue) controls decrement, PC2 controls toggle, reads +5 or 0v													         
	PORTC |= (31<<0);			// 0-4, 5 bits.	                             //Pull-up resistance for buttons
	
	DDRD  |= (1<<DDD5 | 1<<DDD6);                                    //Enable pin outputs D5 and D6 for LEDs
	PORTD |= (1<<PORTD5 | 1<<PORTD6);

	DDRC  |= 1<<DDC5;  PINC |= 1<<PINC5;					         // Debug LED Pin on A5
...
void set_led1_dim() //Same as LED 0
{
	...
	if (OCR0B <10)  OCR0B = 0;     //set upper and lower bounds
...
{
	static uint8_t ON_FLAG = 0U; // make sure user is not holding the button
	// This function should do the same as LED0, but in the opposite direction.
...
void set_led3()
{
	...
	if (OCR2A >245) OCR2A = 255U;
	if (OCR2A <10)  OCR2A = 0U;     //set upper and lower bounds
	
 if ((!(PINC & (1<<0))&& ON_FLAG == 0))
	{
		FFmSDELAY();
		if(OCR2A <= 245U && OCR2A >= 0U){
			OCR2A += 10U;
			ON_FLAG ++;                                              // Discrete step of roughly 2%
		}
	}
